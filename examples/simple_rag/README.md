...
This is a simple example RAG application to showcase how one can configure and use the Retriever component. This example includes:
 ...
 - GPU-accelerated neural-symbolic retrieval with NeMo and Jena Vector integration
...
## Using GPU-Accelerated NeMo Retriever with Jena Vector

This example includes an enhanced NeMo Retriever with Jena Vector capabilities that provides GPU acceleration, knowledge graph integration, and neural-symbolic retrieval. To use this enhanced retriever, you can run the workflow with the `nemo_jena_rag_config.yml` configuration file:

```bash
aiq run --config_file examples/simple_rag/configs/nemo_jena_rag_config.yml --input "What are the latest advancements in GPU tensor cores for AI?"
```

The configuration includes three different retriever types:

1. **Standard NeMo Retriever**: Basic NeMo Retriever functionality
2. **Jena Vector Retriever**: Enhanced retriever with GPU acceleration and knowledge graph integration
3. **Neural-Symbolic Retriever**: Specialized retriever with reasoning capabilities

For more information on using the NeMo Jena Vector Retriever, see the [nemo_jena_README.md](nemo_jena_README.md) file.
The expected output of the above run is:
````console
$ aiq run --config_file examples/simple_rag/configs/milvus_rag_tools_config.yml --input "How do I install CUDA and get started developing with it? Provide example python code"
