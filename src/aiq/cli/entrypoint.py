...
@handle_errors(exceptions_to_catch=(KeyError, ValueError), reraise=True)
def setup_logging(log_level: str):
    """Configure logging with the specified level"""
    numeric_level = LOG_LEVELS.get(log_level.upper())
...
def after_pipeline(ctx: click.Context, pipeline_start_time: float, *_, **__):
    with ErrorContext("CLI completion", raise_on_error=False):
