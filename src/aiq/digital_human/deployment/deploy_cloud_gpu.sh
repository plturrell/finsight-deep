...
# Deploy Digital Human using NVIDIA NIM + Together.ai
set -e
...
# API Keys
export NVIDIA_API_KEY="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"
export TOGETHER_API_KEY="1e961dd58c67427a09c40a09382f8f00e54f39aa8c34ac426fd5579c4effd1b4"

# Create unified cloud deployment
create_cloud_deployment() {
    log "Creating multi-cloud GPU deployment..."
    
    # Create main application with both APIs
    cat > digital_human_cloud.py <<'EOF'
...
create_instructions() {
    cat > DEPLOY_CLOUD_GPU.md <<'EOF'
# Digital Human - Multi-Cloud GPU Deployment

## Available Cloud GPU Services

### 1. NVIDIA NIM (DGX Cloud)
- **API Key**: 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
- **Model**: meta/llama-3.1-8b-instruct
- **Benefits**: Official NVIDIA cloud, optimized for AI
- **Free Credits**: 5,000 API calls

### 2. Together.ai
- **API Key**: 1e961dd58c67427a09c40a09382f8f00e54f39aa8c34ac426fd5579c4effd1b4
- **Model**: meta-llama/Meta-Llama-3.1-8B-Instruct-Turbo
- **Benefits**: Fast inference, competitive pricing
- **Free Credits**: $25 initial credit

## Quick Start

### 1. Test APIs
```bash
python test_cloud_apis.py
```

### 2. Run Locally
```bash
python digital_human_cloud.py
```
...
✅ **Multi-Cloud Redundancy** - Automatic failover between providers
✅ **No Local GPU Required** - All processing in the cloud
✅ **Real-time Performance** - Low latency responses
✅ **Cost Effective** - Use free credits from both services
✅ **Production Ready** - Scalable architecture

## API Selection Options

1. **NVIDIA Only** - Use NVIDIA NIM exclusively
2. **Together Only** - Use Together.ai exclusively  
3. **Auto Mode** - Automatically failover between services

## Performance Comparison

| Feature | NVIDIA NIM | Together.ai |
|---------|------------|-------------|
| Latency | ~200-400ms | ~150-300ms |
| Reliability | 99.9% | 99.5% |
| GPU Type | DGX H100 | A100/H100 |
| Free Tier | 5,000 calls | $25 credit |
...
Since this uses cloud APIs, you can deploy to:
- AWS (no GPU instance needed)
- Google Cloud (no GPU needed)
- Azure (no GPU needed)
- Heroku
- Railway
- Vercel
...
```bash
docker build -f Dockerfile.cloud -t digital-human-cloud .
docker run -p 8000:8000 digital-human-cloud
```

## Cost Optimization

1. **Use Auto Mode** - Balances between providers
2. **Cache Responses** - Reduce API calls
3. **Monitor Usage** - Track credits on both platforms
...
- ✅ Two cloud providers for reliability
- ✅ Free credits on both platforms
- ✅ Immediate deployment
- ✅ Professional UI
- ✅ Real AI capabilities

The dual-cloud approach ensures high availability and impressive performance!
EOF
}
...
main() {
    ...
    info "Multi-cloud GPU deployment created!"
    echo ""
    echo "======================================"
    echo "Digital Human - Cloud GPU Deployment"
    echo "======================================"
    echo ""
    echo "🚀 Quick Start:"
    echo "   python digital_human_cloud.py"
    echo ""
    echo "🧪 Test APIs:"
    echo "   python test_cloud_apis.py"
    echo ""
    echo "🐳 Docker Deploy:"
    echo "   docker-compose -f docker-compose.cloud.yml up"
    echo ""
    echo "☁️ Cloud Providers:"
    echo "   • NVIDIA NIM (DGX Cloud)"
    echo "   • Together.ai (A100/H100)"
    echo ""
    echo "✅ Benefits:"
    echo "   - No local GPU required"
    echo "   - Dual cloud redundancy"
    echo "   - Free credits on both"
